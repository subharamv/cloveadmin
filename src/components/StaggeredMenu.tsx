...
export const StaggeredMenu = ({
  ...
  position?: 'left' | 'right';
  colors?: string[];
  items?: Array<{ label: string; id?: string; link?: string; ariaLabel?: string; onClick?: () => void }>;
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (labelEls.length) {
      gsap.set(labelEls, { y: 20, opacity: 0 });
    }
    if (itemEls.length) {
      gsap.set(itemEls, { x: position === 'left' ? -10 : 10, opacity: 0 });
    }
    if (numberEls.length) {
    ...
    if (footerEl) {
      tl.to(footerEl, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '-=0.4');
    }
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        gsap.set([panel, ...layers], { opacity: 0 });
        busyRef.current = false;
      }
    });
  }, [position]);
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="flex items-center px-4 py-2">
          ...
          >
            ...
            <span ref={textWrapRef} className="sm-toggle-textWrap" aria-hidden="true" style={{ width: '60px' }}>
              <span ref={textInnerRef} className="sm-toggle-textInner">
                {textLines.map((l, i) => (
                  <span className="sm-toggle-line text-[10px] font-black tracking-widest uppercase" key={i}>
                    {l}
                  </span>
                ))}
              </span>
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <div className="sm-panel-inner">
            ...
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={(it.id || it.label) + idx}>
                  <button 
                    className={`sm-panel-item ${activeId === it.id ? 'active' : ''}`}
                    onClick={() => {
                        it.onClick?.();
                        closeMenu();
                    }}
          ...
          {footer && <div className="sm-footer mt-auto">{footer}</div>}
        </div>
      </aside>
    </div>
  );
