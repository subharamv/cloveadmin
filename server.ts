...
// Fallback configuration for sandbox sandbox
...
// MongoDB connection with fast server selection timeout
...
mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 2000 } as any)
  ...
  .catch((err: any) => {
    console.warn('MongoDB connection failed (No local daemon found in preview environment). Falling back to JSON persistent datastore storage for facility logs.', err.message);
...
// Ensure the Visitors/VisitorLogs tabs exist in the Google Sheet
ensureVisitorSheetsSetup().catch((err: any) => {
  console.error('Failed to set up visitor tracking sheets:', err.message);
});

// Ensure the BillPayments/BillPaymentHistory tabs exist in the Google Sheet
...
// Role-based Authorization Middleware Factory (built on top of authMiddleware)
